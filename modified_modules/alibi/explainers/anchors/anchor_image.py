...
from modified_modules.alibi.api.defaults import DEFAULT_DATA_ANCHOR_IMG, DEFAULT_META_ANCHOR
...
class AnchorImageSampler:
    ...
    def __call__(
        ...
        if compute_labels:
            ...
            self.raw_coverage_data = raw_data # 我加了紀錄 coverage_data 的原始資料
            ...
            return [raw_data, covered_true, covered_false, labels.astype(int), data, -1.0, anchor[0]] # 我加上回傳 raw_data
    ...
    def perturbation(
        ...
        image = self.image # 原始影像 (H, W, C)
        segments = self.segments # 影像中每個像素對應的 superpixel ID，形狀 (H, W)
