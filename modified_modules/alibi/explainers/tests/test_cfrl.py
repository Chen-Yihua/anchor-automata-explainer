...
from modified_modules.alibi.explainers import CounterfactualRLTabular
from modified_modules.alibi.explainers.backends.cfrl_base import get_hard_distribution
from modified_modules.alibi.explainers.backends.cfrl_tabular import get_he_preprocessor, split_ohe, \
...
def test_sample_differentiable(dataset, backend):
    ...
    if backend == "tensorflow":
        from modified_modules.alibi.explainers.backends.tensorflow.cfrl_base import to_tensor, to_numpy
        from modified_modules.alibi.explainers.backends.tensorflow.cfrl_tabular import sample_differentiable
        device = None
    else:
        import torch
        from modified_modules.alibi.explainers.backends.pytorch.cfrl_base import to_tensor, to_numpy
        from modified_modules.alibi.explainers.backends.pytorch.cfrl_tabular import sample_differentiable
...
def test_l0_loss(reduction, backend):
    ...
    if backend == 'tensorflow':
        from modified_modules.alibi.explainers.backends.tensorflow.cfrl_base import to_tensor, to_numpy
        from modified_modules.alibi.explainers.backends.tensorflow.cfrl_tabular import l0_ohe
        device = None
    else:
        import torch
        from modified_modules.alibi.explainers.backends.pytorch.cfrl_base import to_tensor, to_numpy
        from modified_modules.alibi.explainers.backends.pytorch.cfrl_tabular import l0_ohe
        device = torch.device("cpu")
...
def test_l1_loss(reduction, backend):
    ...
    if backend == 'tensorflow':
        from modified_modules.alibi.explainers.backends.tensorflow.cfrl_tabular import l1_loss
        from modified_modules.alibi.explainers.backends.tensorflow.cfrl_base import to_tensor, to_numpy
        device = None
    else:
        import torch
        from modified_modules.alibi.explainers.backends.pytorch.cfrl_tabular import l1_loss
        from modified_modules.alibi.explainers.backends.pytorch.cfrl_base import to_tensor, to_numpy
        device = torch.device("cpu")
...
def test_consistency_loss(backend):
    ...
    if backend == 'tensorflow':
        from modified_modules.alibi.explainers.backends.tensorflow.cfrl_tabular import consistency_loss
        from modified_modules.alibi.explainers.backends.tensorflow.cfrl_base import to_tensor, to_numpy
        device = None
    else:
        import torch
        from modified_modules.alibi.explainers.backends.pytorch.cfrl_tabular import consistency_loss
        from modified_modules.alibi.explainers.backends.pytorch.cfrl_base import to_tensor, to_numpy
        device = torch.device("cpu")
...
def test_sparsity_loss(dataset, backend):
    ...
    if backend == 'tensorflow':
        from modified_modules.alibi.explainers.backends.tensorflow.cfrl_tabular import sparsity_loss
        from modified_modules.alibi.explainers.backends.tensorflow.cfrl_base import to_tensor, to_numpy
        device = None
    else:
        import torch
        from modified_modules.alibi.explainers.backends.pytorch.cfrl_tabular import sparsity_loss
        from modified_modules.alibi.explainers.backends.pytorch.cfrl_base import to_tensor, to_numpy
        device = torch.device('cpu')
