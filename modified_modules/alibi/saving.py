...
if TYPE_CHECKING:
    import torch
    import tensorflow
    from modified_modules.alibi.api.interfaces import Explainer
    from modified_modules.alibi.explainers.integrated_gradients import IntegratedGradients
    from modified_modules.alibi.explainers.shap_wrappers import KernelShap, TreeShap
    from modified_modules.alibi.explainers import (
        AnchorImage,
        AnchorText,
        CounterfactualRL,
    ...
    from modified_modules.alibi.prototypes import (
        ProtoSelect
    )

from modified_modules.alibi.version import __version__
...
def _load_IntegratedGradients(path: Union[str, os.PathLike], predictor: 'Union[tensorflow.keras.Model]',
    ...
    from modified_modules.alibi.explainers.integrated_gradients import LayerState
    with open(Path(path, 'explainer.dill'), 'rb') as f:
        explainer = dill.load(f)
...
def _save_IntegratedGradients(explainer: 'IntegratedGradients', path: Union[str, os.PathLike]) -> None:
    from modified_modules.alibi.explainers.integrated_gradients import LayerState
    from modified_modules.alibi.exceptions import SerializationError
...
def _load_AnchorText(path: Union[str, os.PathLike], predictor: Callable, meta: dict) -> 'AnchorText':
    ...
    if sampling_strategy in nlp_sampling:
        ...
        # load language model
        import modified_modules.alibi.utils as lang_model
...
def _save_CounterfactualRL(explainer: 'CounterfactualRL', path: Union[str, os.PathLike]) -> None:
    ...
    from modified_modules.alibi.explainers import CounterfactualRL
...
def _load_CounterfactualRL(path: Union[str, os.PathLike],
    ...
    if explainer.params["backend"] == Framework.TENSORFLOW:
        import modified_modules.alibi.explainers.backends.tensorflow.cfrl_base as backend
    else:
        import modified_modules.explainers.backends.pytorch.cfrl_base as backend  # type: ignore
...
def _load_CounterfactualRLTabular(path: Union[str, os.PathLike],
    ...
    # select backend module
    if explainer.params["backend"] == Framework.TENSORFLOW:
        import modified_modules.alibi.explainers.backends.tensorflow.cfrl_tabular as backend
    else:
        import modified_modules.explainers.backends.pytorch.cfrl_tabular as backend  # type: ignore
